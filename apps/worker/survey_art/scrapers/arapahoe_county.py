...
from survey_art.download import download_all_async
...
async def scrape(
    ...
    doc_urls, cost2, in_tok2, out_tok2 = await _get_document_urls(
        address, parcel_number, doc_filter
    )
    ...
    if not doc_urls:
        return (
            [],
            f"No documents found for {address} in Arapahoe County.",
            total_cost,
            total_in,
            total_out,
        )
