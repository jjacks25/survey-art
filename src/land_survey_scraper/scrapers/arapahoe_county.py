...
async def _get_parcel_number(address: str) -> str | None:
...
async def scrape(
    ...
    parcel_number = await _get_parcel_number(address)
    doc_urls = await _get_document_urls(address, parcel_number, doc_filter)

    if not doc_urls:
        return [], f"No documents found for {address} in Arapahoe County."
